...
/**
 * ⚡ Bolt Optimization: Lifted static arrays to module level to prevent
 * unnecessary re-allocations on every render cycle.
 */
...
export default function HomePage() {
  ...
  /**
   * ⚡ Bolt Optimization: Memoized computed values to prevent expensive
   * filtering and slicing operations on every re-render.
   */
  ...
  const featuredSellers = useMemo(() => {
    return sellers.filter(s => s.featuredStatus).slice(0, 4);
  }, [sellers]);
  ...
  return (
    <div className="flex flex-col gap-8 pb-10">
      ...
      <section className="px-6">
        ...
        >
          {/* ⚡ Bolt Optimization: Used Next.js Image with priority for LCP improvement */}
          <Image
          ...
          {filteredBundles.map((bundle) => (
            <div key={bundle.id} className="min-w-[280px] rounded-3xl overflow-hidden bg-white border border-gray-100 shadow-soft relative group">
              <Link href={`/bundle/${bundle.id}`} className="block relative h-40">
                <Image src={bundle.bundleImage} alt={bundle.bundleName} fill sizes="280px" className="object-cover" />
              </Link>
              <div className="p-4">
            ...
            <div key={p.id} className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-soft group">
              <div className="relative h-32 w-full overflow-hidden">
                <Link href={`/product/${p.id}`} className="block relative w-full h-full">
                  <Image src={p.imageUrl} alt={p.productName} fill sizes="(max-width: 768px) 50vw, 200px" className="object-cover group-hover:scale-105 transition-transform" />
                </Link>
                <button
