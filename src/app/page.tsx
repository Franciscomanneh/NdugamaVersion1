...
export default function HomePage() {
  ...
  // ⚡ Bolt Optimization: Memoize filtered bundles to prevent redundant calculations on every render
  const filteredBundles = useMemo(() => bundles.filter(b =>
    b.bundleName.toLowerCase().includes(searchQuery.toLowerCase())
  ), [bundles, searchQuery]);

  // ⚡ Bolt Optimization: Memoize filtered products to prevent redundant calculations on every render
  const filteredProducts = useMemo(() => products.filter(p =>
    p.productName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    p.category.toLowerCase().includes(searchQuery.toLowerCase())
  ), [products, searchQuery]);

  // ⚡ Bolt Optimization: Memoize featured sellers to avoid filtering on every render
  ...
  return (
    <div className="flex flex-col gap-8 pb-10">
      ...
      <section className="px-6">
        ...
        >
          {/* ⚡ Bolt Optimization: Using Next.js Image with 'priority' for LCP optimization */}
          <Image
        ...
        <div className="flex gap-4 overflow-x-auto px-6 hide-scrollbar pb-4">
          {filteredBundles.map((bundle) => (
            <div key={bundle.id} className="min-w-[280px] rounded-3xl overflow-hidden bg-white border border-gray-100 shadow-soft relative group">
              <Link href={`/bundle/${bundle.id}`} className="relative block h-40">
          ...
          {(searchQuery ? filteredProducts : filteredProducts.slice(0, 4)).map((p) => (
            <div key={p.id} className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-soft group">
              <div className="relative h-32 w-full overflow-hidden">
                <Link href={`/product/${p.id}`} className="relative block h-full">
