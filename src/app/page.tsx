...
export default function HomePage() {
  ...
  // ⚡ Bolt: Memoize filtered bundles to avoid unnecessary recalculations during re-renders.
  // Expected Performance Impact: O(B) reduction on every render (B = number of bundles).
  const filteredBundles = useMemo(() => bundles.filter(b =>
    b.bundleName.toLowerCase().includes(searchQuery.toLowerCase())
  ), [bundles, searchQuery]);

  // ⚡ Bolt: Memoize filtered products to avoid unnecessary recalculations during re-renders.
  // Expected Performance Impact: O(P) reduction on every render (P = number of products).
  ...
  return (
    <div className="flex flex-col gap-8 pb-10">
      ...
      <section className="px-6">
        ...
        >
          {/* ⚡ Bolt: Use Next.js Image with priority for LCP optimization of the hero section. */}
          <Image
      ...
      <section className="px-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          {sellers.filter(s => s.featuredStatus).slice(0, 4).map((s) => (
            ...
            >
              {/* ⚡ Bolt: Use optimized Image for seller profile. */}
              <div className="relative w-full h-24 mb-3 overflow-hidden rounded-xl">
                <Image
        ...
        <div className="flex gap-4 overflow-x-auto px-6 hide-scrollbar pb-4">
          {filteredBundles.map((bundle) => (
            <div key={bundle.id} className="min-w-[280px] rounded-3xl overflow-hidden bg-white border border-gray-100 shadow-soft relative group">
              {/* ⚡ Bolt: Link with block relative for optimized Image. */}
              <Link href={`/bundle/${bundle.id}`} className="block relative w-full h-40">
                <Image src={bundle.bundleImage} alt={bundle.bundleName} fill className="object-cover" />
              </Link>
              <div className="p-4">
            ...
            <div key={p.id} className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-soft group">
              <div className="relative h-32 w-full overflow-hidden">
                ...
                <Link href={`/product/${p.id}`} className="block relative w-full h-full">
                  <Image src={p.imageUrl} alt={p.productName} fill className="object-cover group-hover:scale-105 transition-transform" />
                </Link>
                <button
